...
def SAXDefaultVersion(version):
    """Set the default version of SAX used globally by the
       library. Note that this may not be a good thing to do from
       a library it is better to use xmlSAXVersion() to set up
       specifically the version for a given parsing context. """
...
class xmlDoc(xmlNode):
    ...
    def saveFormatFileTo(self, buf, encoding, format):
        """Dump an XML document to an I/O buffer. NOTE: the I/O buffer
           is closed as part of the call. """
        if buf is None: buf__o = None
        else: buf__o = buf._o
...
class xmlTextReader(xmlTextReaderCore):
    ...
    def Preserve(self):
        """current node being accessed by the xmlTextReader. This is
           dangerous because the underlying node may be destroyed on
           the next Reads. """
